...
if [ -z "$PLATEFORM_PROJET_GIT" ]; then
    PLATEFORM_PROJET_GIT="git://github.com/drupal-composer/drupal-project.git"
fi
...
echo "****  we add in .gitignore file default values from drupal project ****"
if ! grep -q "www.gitignore.io" .gitignore; then
    curl -L -s https://www.gitignore.io/api/drupal >> .gitignore
fi

echo "**** we add env variables ****"
sudo bash -c "cat << EOT > /etc/profile.d/$PLATEFORM_PROJET_NAME_LOWER.sh
# env vars for SFYNFONY platform
export DRUPAL__DATABASE__NAME__ENV__$PLATEFORM_PROJET_NAME_UPPER=$DATABASE_NAME;
export DRUPAL__DATABASE__USER__ENV__$PLATEFORM_PROJET_NAME_UPPER=root;
export DRUPAL__DATABASE__PASSWORD__ENV__$PLATEFORM_PROJET_NAME_UPPER=pacman;
export DRUPAL__TEST__DATABASE__NAME__ENV__$PLATEFORM_PROJET_NAME_UPPER=$DATABASE_NAME_TEST;
export DRUPAL__TEST__DATABASE__USER__ENV__$PLATEFORM_PROJET_NAME_UPPER=root;
export DRUPAL__TEST__DATABASE__PASSWORD__ENV__$PLATEFORM_PROJET_NAME_UPPER=pacman;
...
server {
    ...
    location ~ ^/(index|web.config)\.php(/|\$) {
        ...
        fastcgi_param DRUPAL__DATABASE__NAME__ENV__${PLATEFORM_PROJET_NAME_UPPER} $DATABASE_NAME;
        fastcgi_param DRUPAL__DATABASE__USER__ENV__${PLATEFORM_PROJET_NAME_UPPER} root;
        fastcgi_param DRUPAL__DATABASE__PASSWORD__ENV__${PLATEFORM_PROJET_NAME_UPPER} pacman;
        fastcgi_param DRUPAL__TEST__DATABASE__NAME__ENV__${PLATEFORM_PROJET_NAME_UPPER} $DATABASE_NAME_TEST;
        fastcgi_param DRUPAL__TEST__DATABASE__USER__ENV__${PLATEFORM_PROJET_NAME_UPPER} root;
        fastcgi_param DRUPAL__TEST__DATABASE__PASSWORD__ENV__${PLATEFORM_PROJET_NAME_UPPER} pacman;
    }
